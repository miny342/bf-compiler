...
if ! LC_ALL=C grep -q 'BFC_STAGE9_ERROR' "$work_dir/invalid-stage8-lexer.actual"; then
    echo "stage-9 compiler accepted a single ampersand" >&2
    exit 1
fi
...
if ! LC_ALL=C grep -q 'BFC_STAGE9_ERROR' "$work_dir/invalid-stage8-return.actual"; then
    echo "stage-9 compiler accepted a missing scalar return path" >&2
    exit 1
fi
...
if ! LC_ALL=C grep -q 'BFC_STAGE9_ERROR' "$work_dir/invalid-stage8-whole-array.actual"; then
    echo "stage-9 compiler accepted a whole array as a scalar" >&2
    exit 1
fi
...
if ! LC_ALL=C grep -q 'BFC_STAGE9_ERROR' "$work_dir/invalid-stage8-bounds.actual"; then
    echo "stage-9 compiler accepted an out-of-bounds constant index" >&2
    exit 1
fi
...
if LC_ALL=C grep -q 'BFC_STAGE9_ERROR' "$stage7_bf"; then
    echo "stage-9 compiler rejected stage7_globals.bfc" >&2
    exit 1
fi
if LC_ALL=C grep -q '[^][<>+.,-]' "$stage7_bf"; then
    echo "stage-9 compiler emitted a non-Brainfuck byte for stage7_globals.bfc" >&2
    exit 1
fi
...
if ! LC_ALL=C grep -q 'BFC_STAGE9_ERROR' "$work_dir/invalid-stage8-array-type.actual"; then
    echo "stage-9 compiler accepted mismatched array argument lengths" >&2
    exit 1
fi
...
if LC_ALL=C grep -q 'BFC_STAGE9_ERROR' "$stage8_bf" \
    || LC_ALL=C grep -q '[^][<>+.,-]' "$stage8_bf"; then
    echo "stage-9 compiler rejected or corrupted stage8_aggregates.bfc" >&2
    exit 1
fi
cargo run --quiet --release --manifest-path "$repo_dir/Cargo.toml" \
...
if LC_ALL=C grep -q 'BFC_STAGE9_ERROR' "$snapshot_bf" \
    || LC_ALL=C grep -q '[^][<>+.,-]' "$snapshot_bf"; then
    echo "stage-9 compiler rejected or corrupted stage8_snapshots.bfc" >&2
    exit 1
fi
cargo run --quiet --release --manifest-path "$repo_dir/Cargo.toml" \
...
if ! LC_ALL=C grep -q 'BFC_STAGE9_ERROR' "$work_dir/invalid-stage9-enum.actual"; then
    echo "stage-9 compiler accepted an enum without a zero variant" >&2
    exit 1
fi
...
if ! LC_ALL=C grep -q 'BFC_STAGE9_ERROR' "$work_dir/invalid-stage9-cycle.actual"; then
    echo "stage-9 compiler accepted a recursive struct layout" >&2
    exit 1
fi
...
if LC_ALL=C grep -q 'BFC_STAGE9_ERROR' "$stage9_types_bf" \
    || LC_ALL=C grep -q '[^][<>+.,-]' "$stage9_types_bf"; then
    echo "stage-9 compiler rejected or corrupted stage9_types.bfc" >&2
    exit 1
fi
cargo run --quiet --release --manifest-path "$repo_dir/Cargo.toml" \
...
if LC_ALL=C grep -q 'BFC_STAGE9_ERROR' "$stage9_projections_bf" \
    || LC_ALL=C grep -q '[^][<>+.,-]' "$stage9_projections_bf"; then
    echo "stage-9 compiler rejected or corrupted stage9_projections.bfc" >&2
    exit 1
fi
cargo run --quiet --release --manifest-path "$repo_dir/Cargo.toml" \
...
echo "stage-9 self-host verification passed"
